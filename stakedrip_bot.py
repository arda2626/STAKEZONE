...
DAILY_INTERVAL = 3600*12  # günlük kupon
        ...
        try:
            ...
            for e in events:
                matches.append({
                    ...
                    "sport": sport.lower(),
                    "odds": uniform(1.2,2.0),
                    "confidence": 0,
                    "live": e.get("strStatus","").lower() in ["live","in progress"],
                    "start_time": datetime.strptime(
                        e.get("dateEvent","") + " " + (e.get("strTime","12:00:00")),
                        "%Y-%m-%d %H:%M:%S"
                    ).replace(tzinfo=timezone.utc) if e.get("dateEvent") else utcnow(),
...
async def daily_coupon(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    log.info("Günlük kupon tahminleri gönderildi")

async def weekly_coupon(ctx: ContextTypes.DEFAULT_TYPE):
    matches = await fetch_all_matches()
    upcoming = [m for m in matches if m.get("start_time") < utcnow() + timedelta(days=7)]
    predictions = sorted([ai_for_match(m) for m in upcoming], key=lambda x: x["confidence"], reverse=True)[:10]
    if predictions:
        text = "\n".join([f"{p['home']} vs {p['away']} • {p['odds']} • {p['prob']*100:.0f}%" for p in predictions])
        await ctx.bot.send_message(CHANNEL_ID, f"📅 HAFTALIK KUPON 🔥\n{text}")
    log.info("Haftalık kupon tahminleri gönderildi")

async def kasa_coupon(ctx: ContextTypes.DEFAULT_TYPE):
    matches = await fetch_all_matches()
    upcoming = [m for m in matches if m.get("start_time") < utcnow() + timedelta(hours=48)]
    predictions = sorted([ai_for_match(m) for m in upcoming], key=lambda x: x["confidence"], reverse=True)[:3]
    if predictions:
        text = "\n".join([f"{p['home']} vs {p['away']} • {p['odds']} • {p['prob']*100:.0f}%" for p in predictions])
        await ctx.bot.send_message(CHANNEL_ID, f"💰 KASA KUPONU 🔥\n{text}")
    log.info("Kasa kupon tahminleri gönderildi")

async def check_results(ctx: ContextTypes.DEFAULT_TYPE):
    log.info("check_results: maç sonuçları kontrol ediliyor (placeholder)")
...
async def main():
    ...
    jq.run_repeating(daily_coupon, interval=DAILY_INTERVAL, first=30)
    jq.run_repeating(weekly_coupon, interval=86400, first=60)
    jq.run_repeating(kasa_coupon, interval=86400, first=120)
    jq.run_repeating(check_results, interval=300, first=30)
    jq.run_repeating(daily_summary, interval=86400, first=60)
